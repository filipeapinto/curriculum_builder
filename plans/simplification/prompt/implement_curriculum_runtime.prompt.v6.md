# Implement the Missing Curriculum Runtime — Prompt v6
...
2. Complete the single bounded capability-enablement cycle below, validate the resulting
   contracts, and prove the live prerequisites before starting a costly L01 attempt.
3. Start L01 from a new, empty output root beneath `ENGINE/outputs/` and preserve the attempt as
   evidence.
...
5. Keep the failed L01 output unchanged. If budget and convergence conditions still
   permit another attempt, select a new empty output root beneath `ENGINE/outputs/` and rerun L01 from
...
LAB_ID       = L01
EVIDENCE_PARENT      = ENGINE/outputs
TASK_ROOT            = EVIDENCE_PARENT/runtime_task_v6
...
Reject any output root that does not resolve beneath `ENGINE/outputs/`. Before each outer-loop attempt, the
...
1. **Development interval.** No curriculum run is active. Writes inside `ENGINE` are
   allowed only for the reusable runtime, its focused tests, and the exact capability-
   enablement files named below. Record the dirty-worktree baseline first and never
...
- Maximum ImageGen proof calls: **1**. No generated image may be used or shipped in L01;
  L01 must use deterministic or verified official visuals.
- Maximum capability-enablement cycles: **1**.
...
During development, the complete repository write allowlist is:
...
The three policy files may change only inside the single capability-enablement cycle
defined below. Do not edit the meta-prompt, curriculum files, controller policy, limits,
check inventory, task taxonomy, schemas, or unrelated tests. If the required capability
facts cannot be represented and proven within this allowlist and one cycle, return the
bounded live-blocked outcome after completing the runtime's deterministic and simulated
tests. Do not expand scope again.
...
- Pass both model and effort explicitly through every route. For Gemini CLI, effort is
  enforced by the hashed run-local settings mechanism below rather than a nonexistent
  command-line effort flag. A route without a validated policy-to-provider effort
  mapping and proof that the invoked CLI loaded it is a live-prerequisite failure.
...
## Deliverable 2 — one bounded capability-enablement cycle

The active contract requires a proven cross-family judge and requires every declared
route either to be proven or removed when unavailable. Satisfy those existing rules in
one explicitly authorized development cycle before freezing the contracts. This is the
only route-policy work in scope.

### Verified Gemini CLI effort mechanism

This prompt was authored after inspecting the installed Gemini CLI **0.24.5**, its
bundled configuration documentation, default model configurations, SDK types, and
headless-output documentation. The verified facts are:

- `gemini --help` exposes `-m/--model` but no direct effort flag.
- Gemini CLI loads settings in layers and supports an override path for its system
  settings through `GEMINI_CLI_SYSTEM_SETTINGS_PATH`.
- `modelConfigs.aliases` may be selected with `-m` and may set
  `generateContentConfig.thinkingConfig` using either `thinkingLevel` or
  `thinkingBudget`.
- The installed SDK's discrete `ThinkingLevel` enum contains `LOW` and `HIGH`; therefore
  `HIGH` is the maximum discrete provider value for a compatible model.
- Headless `--output-format stream-json` emits an `init` event containing the model
  actually executed.

Re-derive and hash the installed executable and bundled documentation at execution
time. If the installed version differs, re-establish these facts from that version's
own bundled documentation and code before any live call. Never infer a control from a
model name or copy a numeric budget from another model.

1. Before changing anything, validate and hash `policy/routes.v1.yaml`, the model
   registry, and the routing policy. Copy their original bytes and hashes into
   `TASK_ROOT` as evidence.
2. Treat `RESEARCH` exactly as the active meta-prompt defines it: an accepted, recorded
   ...
   Do not add a research route and do not claim the total external capability set is
   fully proven while that divergence remains.
3. The existing final-acceptance policy requires reasoning effort `max`. Select a Gemini
   model only when its installed or primary official documentation identifies an
   enforceable maximum thinking control. Map policy `max` to that provider maximum:
   use `thinkingLevel: HIGH` only for a compatible model whose discrete enum is proven
   to make `HIGH` the maximum, or use a numeric `thinkingBudget` only when that exact
   model's official documentation proves the permitted maximum. Record the mapping and
   its evidence. Do not treat the CLI's default 8192-token configuration as a maximum.
4. Create an isolated run-local Gemini settings file beneath `TASK_ROOT` for the proof.
   Point `GEMINI_CLI_SYSTEM_SETTINGS_PATH` at that exact file; never edit user, project,
   or machine-global Gemini settings. Define a model alias whose key and underlying
   model are the same explicit model id, configure the proven maximum thinking control,
   disable tool discovery/execution, and validate the resolved alias with the installed
   Gemini configuration loader. Record the settings bytes, SHA-256, resolved model
   configuration, configuration-layer audit, CLI version, and binary hash.
5. Make at most one minimal real Gemini CLI proof call from an isolated directory under
   `TASK_ROOT`, with the run-local system-settings override, explicit `-m` model id,
   non-interactive prompt, sandboxing, no authorized tools, and
   `--output-format stream-json`. Require exactly one successful result, zero tool-use
   events, and an `init.model` equal to the decided model. The proof receipt must bind
   the command, working directory, settings hash, policy effort `max`, provider control,
   mapping evidence, and executed model. Help output or a response without these facts
   is not proof.
6. Only after the settings resolver and live call prove model and effort, add one
   generic cross-family-judge route with the exact invocation and run-local settings
   parameters. Add only the genuinely executed model to
   `policy/routing/model_registry.v1.yaml`, including the required prose pattern and
   `reasoning_efforts: [max]`, and make the minimum eligibility change in
   `policy/routing/routing_policy.v1.yaml` for the existing final-acceptance task. Do not
   enable that model for unrelated task classes.
7. Give the existing ImageGen route at most one minimal proof attempt. If it succeeds,
   record its exact callable route and proof. If it cannot be proven, first establish
   from the L01 manifest and visual plan that no required L01 artifact depends on it,
   then remove it exactly as `policy/routes.v1.yaml` instructs for an unavailable route.
   Preserve the original entry, failed proof evidence, dependency decision, and policy
   clause in `TASK_ROOT`. This removal is capability-set maintenance required by the
   authoritative policy; it must occur before `ROUTE-PROVEN` runs and may never be used
   to remove a route required by L01. If L01 requires the route, keep it and fail the
   capability cycle. Never edit `ROUTE-PROVEN` or its quantification.
8. Add focused tests under `tests/runtime/**` proving that an unavailable unused route
   follows the declared removal rule, a required route cannot be removed, the
   cross-family route cannot be declared without real proof, the registered and
   executed model agree, the `max` effort mapping resolves from the hashed run-local
   settings, a missing/overridden/unsupported thinking control is refused, no user or
   global Gemini settings are modified, and no policy file outside the exact allowlist
   changed.
9. Validate the three changed manifests against their existing schemas. Run all directly
   affected repository gates and negative controls. Do not change a schema, gate, check
   inventory, controller rule, or fixture to make the policy edits pass.
10. If the cycle succeeds, record and freeze the new policy hashes. During every L01
   attempt, make one real minimal preflight call on every remaining declared route and
   require `ROUTE-PROVEN`. If the cycle fails, preserve its evidence, complete the
   runtime's deterministic and simulated tests, and return the appropriate live-blocked
   outcome without starting L01.

There is no second capability-enablement cycle. Do not add providers, fallback chains,
route frameworks, generated-image features, or any policy change beyond the exact work
above.
...
For every acceptance attempt, after the logger passes and before cross-family preflight,
the controller must create a fresh run-local Gemini system-settings file beneath that
attempt's `OUTPUT_ROOT`, reproduce the proven `max` effort mapping, validate and hash the
resolved model configuration, and point `GEMINI_CLI_SYSTEM_SETTINGS_PATH` to it. Both the
preflight and judge call must run from the authorized external root, use the explicit
model id, emit stream JSON, record `init.model`, and produce zero tool-use events. Never
reuse the proof directory's settings file or depend on user/global Gemini settings.
...
- Gemini run-local settings precedence, alias resolution, `max` effort mapping,
  settings-hash binding, stream `init.model` equality, and tool-use refusal.
...
- `RUNTIME_CORE_VERIFIED_LIVE_BLOCKED`: the runtime's deterministic and simulated tests
  pass, but the single capability-enablement cycle cannot produce valid, declared, and
  proven live prerequisites. Do not start L01 and do not call the runtime accepted.
...
8. Exactly one proven, declared cross-family judge runs per pass with rubric, randomized
   order, structural isolation, explicit model equality, and a resolving receipt for
   the run-local `max` effort mapping and settings hash.
...
- Task-ledger path; clean-attempt count; cumulative wall time, model calls,
  cross-family calls, source fetches, ImageGen proof calls, and storage.
- Capability-enablement changes and proof receipts; before/after hashes and exact diffs
  for the three authorized policy files; the recorded `RESEARCH` divergence; and whether
  a live L01 attempt was permitted.
- Gemini CLI version and binary hash; bundled configuration evidence; policy-to-provider
  effort mapping; run-local settings paths and hashes; resolved aliases; and every
  `init.model`/tool-use audit.
