...
Supersedes `simplification.prompt.v1.md`. v1 refused to start without two arguments —
`--circuit-mode` and `--signoff`. Both decisions are now taken and recorded in the
plan (`../plan/simplification.plan.v3.md` §4): the domain data is **composed**,
declared by the curriculum as `circuit_policy: composed`, and there is **no
in-pipeline sign-off** — the output is a draft, reviewed downstream. This prompt
therefore takes no arguments and never re-asks either question.

## Goal

Implement `../plan/simplification.plan.v3.md` phases 0–6, so that two things hold at
once:
...
Read `AGENTS.md`, the complete v3 plan, and `../research/conclusions.v1.md`. For each
...
- **`circuit_policy: composed` is declared, not assumed.** Phase 2 adds the declaration
  to `arduino_kit_curriculum.v4.yaml` alongside the verifier. If phase 6 finds it
  absent, that is a phase-2 defect, not a licence to default.
...
Return `APPROVED` only when: stage A passes at every phase; the L01 test passes all
eight conditions; the six extracted rules resolve from outside `meta_prompt/`; and the
report states, separately and in these terms, (a) that the pipeline produced one unit,
(b) that the unit is a draft pending downstream human review, per the plan's §4(b),
(c) that L01 did not exercise the powered-circuit path, and (d) that genericity is
structurally enforced and not yet demonstrated.
