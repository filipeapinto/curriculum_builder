# Migrate External Run Evidence Into ENGINE/outputs — Prompt v1
...
# 6. Scope check: nothing outside the authorized paths changed
git status --short | grep -vE '^\?\? outputs/|^ M plans/simplification/prompt/implement_curriculum_runtime\.prompt\.v6\.md$' \
  | (! grep .) && echo "SCOPE_OK"
```

(Steps 3 and 6 will also need to tolerate the pre-existing unrelated dirty files this
task must not touch — `docs/how_it_works.md`, `docs/how_it_works.png`,
`docs/how_it_works.typ`, `docs/infographic.prompt.v1.md`, `readme.md`, and the
untracked `docs/deprecated/`, `docs/png/`, `docs/prompts/`, `plans/contract_v2/`,
`plans/skill_conversion/`, and the sibling `implement_curriculum_runtime.prompt.v{1,2,3,4,5}.md`
files. Widen the `grep -v` pattern in step 6 to exclude exactly those, and no more.)
