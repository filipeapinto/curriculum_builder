# Implement the Missing Curriculum Runtime — Prompt v5
...
TASK_ROOT            = EVIDENCE_PARENT/runtime_task_v5
...
- Pass both model and effort explicitly through every route that exposes enforceable
  controls. For a cross-family route, do not claim an effort was executed unless the
  real invocation proves how it was selected or enforced; an unprovable executed effort
  is a live-prerequisite failure, not a value to invent.
...
3. Make at most one minimal real Gemini CLI proof call for the cross-family judge. The
   invocation must be non-interactive, name the model explicitly, restrict tools and
   writable paths, request structured output, and reveal enough evidence to record the
   model actually executed. Help output, installation, and a successful response that
   does not identify the executed model are not proof.
4. Only after that call proves the facts, add one generic cross-family-judge route with
   its exact executed command and evidence. Add only the genuinely executed model to
   `policy/routing/model_registry.v1.yaml`, including the required prose pattern, and
   make the minimum eligibility change in `policy/routing/routing_policy.v1.yaml` for
   the existing final-acceptance task. Do not enable that model for unrelated task
   classes. Do not invent an effort control; if executed effort cannot satisfy the
   controller contract, record `LIVE_PREREQUISITE_BLOCKED`.
5. Give the existing ImageGen route at most one minimal proof attempt. If it succeeds,
...
6. Add focused tests under `tests/runtime/**` proving that an unavailable unused route
   ...
   executed model agree, and no policy file outside the exact allowlist changed.
7. Validate the three changed manifests against their existing schemas. Run all directly
...
8. If the cycle succeeds, record and freeze the new policy hashes. During every L01
