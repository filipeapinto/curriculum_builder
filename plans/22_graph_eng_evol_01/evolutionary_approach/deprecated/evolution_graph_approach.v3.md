...
- Supersedes: `plans/22_graph_eng_evol_01/evolution_graph_approach.v2.md`
...
7. an immutable candidate/review-bundle manifest and worktree protocol;
8. an official-plugin provider/auth/metering preflight;
9. a schema-bound Codex review receipt, protected append-only store, and
   deterministic verdict parser;
10. a deterministic prompt-graph compiler;
11. a minimal Claude-generator/Codex-reviewer/repair vertical slice;
12. the candidate selection and promotion controller; and
13. the final expanded Plan 22 prompt graph.
...
paths. It does not prove that v2's decisive gate was sound. V3 therefore keeps
the working bridge, removes native `/codex:review` from decisive acceptance,
adds exact frozen-target verification, restores evaluator-owned holdouts, and
defines schema-bound, tamper-evident receipts. The complete v3 protocol still
requires implementation and its own vertical-slice test before promotion.
