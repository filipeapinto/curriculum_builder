# Run 26 LangGraph Curriculum Factory Postmortem v1
...
Plan 26 nevertheless made Gemini a mandatory production dependency.
...
- The corrected specification must require subscription-backed Claude authoring/repair and subscription-backed Codex judgment.
- Gemini commands, models, credentials, authorization fields, routes, and fallbacks must be prohibited from the production contract.
...
### PM-05 — The exact Gemini model was hardcoded without a documented user or policy decision

Severity: **High**  
Status: **Open**

`gemini-3-pro-preview` first appeared as an implementation default and was later frozen into Plan 26 and `model_jobs.v1.yaml`. The repository contains no direct user instruction choosing that model and no adequate decision record establishing why a preview model was appropriate, available under the user's access, or supportable for production.

Impact:

- The product was bound to an unapproved, unauthenticated provider/model.
- Model availability and lifecycle risk were converted into production prerequisites without consent.

Required correction:

- Remove the model from the corrected specification and active configuration requirements.
- Require model identity and entitlement to be proven under the actual subscription-backed drivers.
...
### PM-09 — Plan 26 authorized Google transmission without a valid provider decision

Severity: **Critical**  
Status: **Open**

The specification granted the `google` provider transmission of actual unit/workbook artifacts, shipped PDFs, deterministic evidence, and every rasterized page required by M05/M07. Because the provider selection itself was unauthorized, this data-egress authorization was unsupported as well.

Impact:

- The spec expanded the data-sharing boundary to a third provider without a valid requirements basis.
- Had credentials existed, actual curriculum artifacts could have been transmitted under a mistaken authorization model.

Required correction:

- Remove Google/Gemini transmission authority.
- Re-derive and explicitly approve provider-specific data boundaries for Claude and Codex only.

### PM-10 — The provider roles were inverted relative to the recorded target architecture

Severity: **Critical**  
Status: **Open**

The recorded target architecture is Claude authoring and Codex judgment. Plan 26 instead assigned Codex to authoring/repair and Gemini to judgment. This is not a one-line reviewer substitution: replacing Gemini with Codex while retaining Codex authoring would violate the different-family rule.

Impact:

- Correcting the spec requires a complete, consistent reassignment of all eight model jobs and their transport/authentication contracts.

Required correction:

- Assign Claude/Anthropic to the authoring, research, visual, repair, and assembly roles that generate or mutate product content.
- Assign Codex/OpenAI to the independent review/judgment roles.
- Revalidate any exceptional role instead of blindly performing a provider-name replacement.
...
### PM-21 — Passing tests were overinterpreted as product readiness

Severity: **High**  
Status: **Open communication/acceptance issue**

The final suite passed 1,246 tests with one environment-only skip, but N60 produced no curriculum artifact and ran neither the one-unit nor full-release live proof. Most graph end-to-end proofs used fake/scripted transports, appropriately for implementation testing but insufficient for product activation.

Impact:

- “The rework is complete” and “built and verified” obscured that no authorized live product execution occurred.
- The test count appeared to compensate for missing product evidence.

Required correction:

- State implementation-test success and product activation as separate facts.
- Never summarize `NOT_AVAILABLE` live proof as merely “awaiting authentication” when the provider itself is unauthorized.

### PM-22 — N60 recommended remedies that contradicted the user constraint

Severity: **Critical**  
Status: **Open record correction**

N60 acknowledged that only Claude Code and ChatGPT Pro/Codex subscriptions exist and no billed API key exists, yet suggested signing into Gemini or provisioning `GEMINI_API_KEY` as resolution paths.

Impact:

- The result converted a requirements conflict into a user credential task.
- It risked pushing the user toward an explicitly rejected billing/provider model.

Required correction:

- Amend/supersede the result interpretation: the blocker is specification selection, not missing user action.
