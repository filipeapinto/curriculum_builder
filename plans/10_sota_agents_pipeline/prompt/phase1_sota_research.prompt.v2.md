...
## Source verification (required before citing)

Before citing any source in a `.md` file or in `sota_agents.v1.json`, fetch
it (e.g. via WebFetch) and confirm it resolves to real content that supports
the claim. Discard and do not cite any source that fails to resolve, 404s, or
doesn't back the claim — do not include unverified or from-memory citations.
...
[
  {
    ...
    "sources": ["array of URLs or citations backing this recommendation, all verified per above"]
  }
]
```
...
Log every action you take (searches run, files read, files written, sources
verified and their result) to
