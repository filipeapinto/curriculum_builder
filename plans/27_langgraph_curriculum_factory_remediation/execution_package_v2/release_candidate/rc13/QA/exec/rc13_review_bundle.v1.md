# Run 27 RC13 — graph v9 live-defect recovery review bundle
...
## RC12 blocker correction

RC12 independently found that v9 live-proof/final-audit verification commands omitted the fresh state namespace. Graph v9 now passes `--state-dir plans/27_langgraph_curriculum_factory_remediation/.run27_state_v9` explicitly in N70, N80, and N90 controller commands. A direct package regression asserts all three bindings. The shared historical controller default remains unchanged so old N10 evidence continues to validate; every v9 mutation and read in this run uses the explicit state directory.
