# RC1 review bundle — Run 27 execution package v2 release candidate

## Why this document exists

This is the single artifact submitted to `qa-gate-codex-run` for this session. It is
**not** a summary or a pointer file — every file below whose logic PKGV2-T01, T05,
T06, and T10 depend on is embedded verbatim, in full, so that Codex's judged content
*is* the reviewed logic itself, not a description of it or a `--ground` source held
outside the chain.

This bundle replaces a defective session setup from a prior QA attempt
(`plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/QA/`,
session `019ffc3b-6379-7ef3-839d-759ed5c7fc9c`), in which the four files embedded
below (`controller/scan_node.py`, `tools/validate_plan_v2.py`,
`tools/validate_result_v2.py`, `tests/test_execution_package_v2.py`) were passed as
`--ground` sources instead of as reviewed artifact content. Grounding sources are
required to stay byte-identical for the life of a session; these four were
legitimately edited between rounds as their own logic was the subject of the fixes
PKGV2-QA-001 and PKGV2-QA-002 required. That produced `GROUNDING_CHANGED` at every
round after round 1, and `verify` correctly reported `QA_FAILED` /
`INTEGRITY_BREACH` even though the round-by-round content review (3 real findings,
legitimately fixed, then a round-4 PASS) was itself sound. That failed session's
directory and its `INTEGRITY_BREACH` verdict are preserved exactly as they were and
are **not** reopened, edited, or overridden by this bundle or this session.

This bundle is that same round-4-passing content — byte-for-byte identical, confirmed
by SHA-256 against the live `execution_package_v2/` files at snapshot time (see
`manifest.v1.json` beside this file) — resubmitted the correct way: as the single
versioned artifact a `qa-gate-codex-run` session reviews, so nothing whose
correctness matters to the criteria is reachable only as unreviewed, immutable-but-
unjudged grounding.

Read every section below as source you are being asked to judge, exactly as if you
had opened each file directly at its path (given for traceability only — judge the
text below, not a live version at that path, since this bundle is the frozen,
...
Execution graph (round-4 passing artifact of the failed session). SHA-256 at snapshot time: `e1e7f0219e6c91fe3717eed0ecff0309738807b3da71d5a38470060bb5365f28`.
...
nodes:
  N00_SPEC_APPROVAL_GATE:
    prompt: plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/prompts/N00_spec_approval_gate.prompt.v3.md
    depends_on: []
    writes:
  ...
  N20_PROVIDER_TRANSPORT:
    prompt: plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/prompts/N20_provider_transport.prompt.v3.md
  ...
  N30_PREFLIGHT_EGRESS:
    prompt: plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/prompts/N30_preflight_egress.prompt.v3.md
...
Node-scoped / complete-tree forbidden-reference scanner entry point. SHA-256 at snapshot time: `a68141343848f2cfdb5f4656f103c84c2d79a7883f01bf5bdd95c078ce33f11f`.
...
Package-v2 structural plan validator. SHA-256 at snapshot time: `18afb393c19adcf7eb1191b7f8d4286ee62134ce7ee2d6654e5e7bc1155ffc74`.
...
Package-v2 node-result validator. SHA-256 at snapshot time: `6b6b1c261864a95e009a663224bce127527c29ad4fd414e9c9c2013f9d40d89a`.
...
Automated proof suite for PKGV2-T01..T09. SHA-256 at snapshot time: `732c0dc36b5553fd309acf0782c64e710caeb908398c3afcc5d7067d94a43e9e`.
...
## `prompts/N00_spec_approval_gate.prompt.v3.md`

N00 gate prompt. SHA-256 at snapshot time: `b86d217704348b75ed25040fc01747da8b7fe37107ef8bbd981f6f7eb08825e3`.
...
5. Require `execution_package_v2/implementation.graph.v1.yaml` to exist, to
...
## `prompts/N20_provider_transport.prompt.v3.md`

N20 transport implementation prompt. SHA-256 at snapshot time: `48ff86fb12de767c14700a556c825da10b8b03050e0b7e47c6d772fd55303198`.
...
--graph execution_package_v2/implementation.graph.v1.yaml`**, explicitly
...
5. Mechanically search production code/config/policy for the retired
   ...
   `python3 plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/controller/scan_node.py --node N20_PROVIDER_TRANSPORT --graph plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/implementation.graph.v1.yaml`
...
## `prompts/N30_preflight_egress.prompt.v3.md`

N30 preflight/egress-consumer prompt. SHA-256 at snapshot time: `44917fa6ef8aa1d208e88d35a0473f89e10c4a03dcdc49f959601d7d96fb33c9`.
...
correction of N20-F02 — see `N20_provider_transport.prompt.v3.md`). Correct
...
(`python3 plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/controller/scan_node.py --node N30_PREFLIGHT_EGRESS --graph plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/implementation.graph.v1.yaml`)
to remain green.
