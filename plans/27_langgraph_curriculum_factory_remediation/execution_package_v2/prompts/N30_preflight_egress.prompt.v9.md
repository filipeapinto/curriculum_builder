...
This generation additionally freezes the curriculum's complete executable domain contract at D02: artifact schema, manifest schema, config, calibration, verifier entry point/invocation, and every accept/reject fixture. D07 stages those exact hash-verified bytes and admitted source claims for M02. Drift, path escape, missing fixtures, or an unproven/incomplete declaration fails before model authoring.
...
correction of N20-F02 — see `N20_provider_transport.prompt.v9.md`). Correct
...
result-namespace defect fixed in `implementation.graph.v9.yaml` (the defect
...
the earlier v8 change was a mechanical gate-lineage rename; v9 also carries the live-proven contract-freezing repair, following the same discipline
`N30_preflight_egress.prompt.v6.md` itself used for its
`N30_preflight_egress.prompt.v5.md` correction: only this file's own
`--graph` value now names `implementation.graph.v9.yaml` instead of
`implementation.graph.v6.yaml`, the same rename `implementation.graph.v9.yaml`'s
...
(`python3 plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/controller/scan_node.py --node N30_PREFLIGHT_EGRESS --graph plans/27_langgraph_curriculum_factory_remediation/execution_package_v2/implementation.graph.v9.yaml`)
to remain green.
