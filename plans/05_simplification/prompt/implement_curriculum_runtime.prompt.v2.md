# Implement the Missing Curriculum Runtime — Prompt v2
...
## Goal, acceptance test, and development loop
...
1. Implement the smallest reusable runtime capable of attempting the complete L01 run.
2. Start an L01 test from a new, empty, explicitly versioned output root.
3. Run the complete workflow through the runtime and preserve the result as test
   evidence.
4. If the attempt fails, diagnose the failure as a defect or missing capability in the
   reusable runtime, controller logic, generic orchestration, or active contract
   implementation.
5. Improve the reusable implementation. Do not hand-edit, patch, or curate the failed
   L01 output to manufacture a pass.
6. Keep the failed attempt unchanged, select the next empty output-root version, and
   rerun L01 from the beginning. Do not resume or reuse artifacts from a failed
   acceptance attempt.
7. Repeat steps 2–6 until a clean L01 attempt passes every acceptance criterion.

The controller's own bounded retries, targeted revisions, checkpoints, and resume
behavior operate inside a single test attempt and are themselves under test. They do not
replace the outer development loop: after any implementation change, the proof must be a
new clean L01 run.
...
the ranked repository contracts, record assumptions, and keep working through clean L01
attempts until the runtime passes the acceptance test. A failed attempt or an internal
terminal state feeds the development loop; it is not completion of this task. Never
weaken a schema, fixture, gate, verifier, evidence rule, or acceptance criterion to
obtain a pass.
...
LAB_ID       = L01
OUTPUT_ROOT_TEMPLATE = outputs/arduino_kit_runtime_test_v<N>
FIRST_OUTPUT_ROOT    = outputs/arduino_kit_runtime_test_v1
```

Before each outer-loop attempt, explicitly select the first confirmed nonexistent
version of `OUTPUT_ROOT_TEMPLATE` and pass that exact path to the runtime as
`OUTPUT_ROOT`. This explicit development-loop selection is not runtime auto-increment.
The runtime itself must refuse an existing supplied root under
`PRECONDITION-OUTPUT-ROOT-EXISTS`; it must never overwrite, merge, delete, or choose a
different root.

Preserve `outputs/arduino_kit_run_v1` unchanged as evidence from an earlier mistaken
attempt. Preserve every pre-existing user change in the worktree. Never reset, stash,
discard, overwrite, or commit unrelated changes. Every new acceptance attempt must use
a new empty output-root version; never overwrite an earlier attempt.
...
attempt unchanged, fix the reusable implementation, and start L01 again using the next
...
A failed attempt is diagnostic evidence, not completion. Preserve it unchanged, improve
the reusable runtime, and execute the next clean L01 attempt. Do not call the runtime
implemented or L01 accepted until one clean attempt satisfies every criterion.
