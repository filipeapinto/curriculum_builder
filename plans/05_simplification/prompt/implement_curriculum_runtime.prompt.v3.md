# Implement the Missing Curriculum Runtime — Prompt v3
...
2. Prove the declared live prerequisites before starting a costly L01 attempt.
...
LAB_ID       = L01
TASK_ROOT            = /private/tmp/curriculum_builder_runtime_task_v3
TASK_LEDGER          = /private/tmp/curriculum_builder_runtime_task_v3/task_ledger.json
OUTPUT_ROOT_TEMPLATE = /private/tmp/curriculum_builder_runtime_task_v3/attempt_v<N>
FIRST_OUTPUT_ROOT    = /private/tmp/curriculum_builder_runtime_task_v3/attempt_v1
```

Canonicalize `ENGINE` and each proposed `OUTPUT_ROOT` before writing. Reject any output
root equal to or nested beneath `ENGINE`. Before each outer-loop attempt, the
...
1. **Development interval.** No curriculum run is active. Writes inside `ENGINE` are
   allowed only for the reusable runtime, its focused tests, and the narrowly bounded
   capability reconciliation described below. Record the dirty-worktree baseline first
   and never touch unrelated paths.
...
During development, the ordinary write allowlist is `runtime/**` and
`tests/runtime/**`. The single capability-reconciliation cycle below may additionally
change only `policy/routes.v1.yaml`, `policy/routing/model_registry.v1.yaml`, and
`policy/routing/routing_policy.v1.yaml`, plus focused tests beneath `tests/runtime/**`.
Do not edit the meta-prompt, curriculum files, controller policy, limits, check
inventory, schemas, or unrelated tests. If the required live facts cannot be represented
without widening this allowlist, return `LIVE_PREREQUISITE_BLOCKED` instead of expanding
scope.
...
## Deliverable 2 — bounded capability reconciliation and readiness

Do not silently require undeclared routes. Before the first live L01 attempt, perform
one bounded reconciliation cycle against the active contracts:

1. Treat `RESEARCH` exactly as the active meta-prompt defines it: an accepted, recorded
   divergence used for primary-source reads, not a route in `policy/routes.v1.yaml`.
   Do not add a research route and do not claim the external route set is fully proven
   while that divergence remains. Count every source fetch against the task envelope.
2. L01 uses no generated images. Because the declared ImageGen route is unproven and
   `ROUTE-PROVEN` quantifies over every declared route, remove that unused route under
   the route policy's own failure rule. Do not invoke ImageGen and do not replace the
   required deterministic or verified-official visuals with generated imagery.
3. The cross-family judge is required but its CLI route is not currently declared.
   Make one minimal real Gemini CLI proof call. If it succeeds, add one generic
   cross-family-judge route with the exact executed command and evidence, add the
   genuinely executed model to the existing registry, and make only the minimum routing
   eligibility change necessary for the judge task. Validate all three existing
   manifests and run focused negative tests. Never invent a model id, effort control, or
   returned result.
4. Do not change a schema to accommodate the route. If the existing schemas cannot
   represent the proven invocation, or if Gemini cannot be proven within this one
   reconciliation cycle, record `LIVE_PREREQUISITE_BLOCKED`. Continue and finish the
   runtime's deterministic and simulated tests, but do not spend live L01 budget.
5. Once the declared set is coherent, make one real minimal preflight call on every
   declared route before each L01 attempt. This includes the Codex worker with explicit
   selected model and effort, the cross-family judge, Pandoc/Typst PDF rendering, and
   Poppler rasterization. Help output, installed binaries, and configuration are not
   proof.

This reconciliation is a prerequisite with a one-cycle cap, not an open-ended second
project. Do not add providers, route abstractions, fallback chains, or generated-image
support beyond the exact facts required above.
...
- `RUNTIME_CORE_VERIFIED_LIVE_BLOCKED`: the runtime's deterministic and simulated tests
  pass, but the one-cycle capability reconciliation cannot produce the declared and
  proven prerequisites. Do not start L01 and do not call the runtime accepted.
...
partial acceptance. Only `ACCEPTED` proves the runtime end-to-end.
...
- Capability reconciliation changes and proofs, including the recorded `RESEARCH`
  divergence and whether a live L01 attempt was permitted.
