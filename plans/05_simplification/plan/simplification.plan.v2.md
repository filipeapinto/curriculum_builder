...
**Supersedes:** `plan/deprecated/simplification.plan.v1.md`, which corrected the
electronics pipeline and never addressed genericity. Its defect ids (E1, E2, V1–V3,
S1) are carried forward here and re-cast; the analysis behind them stands.
...
## 4. The decision this plan cannot make

**Two, and they are for a human.**

**(a) For electronics, does the domain data get created or composed?**

| Option | `CIRCUIT` does | Cost |
|---|---|---|
| **composed** | selects from a small library of pre-vetted, simulated circuits, signed off once | removes the 8.15%; requires ~35 circuits of expert work, once, not per run |
| **created, gated** | designs, then ERC **and** simulation **and** named human sign-off before ship | keeps the ambition; every unit blocks on a human, so the run is not unattended |
| **created, ungated** | designs, as today | no evidence supports it; the one located precedent was withdrawn after telling children where to find matches and knives |

**Recommendation: composed.** It is also the only option under which an unattended run
stays coherent — the gated option puts a human in the loop per unit anyway.

**(b) Is there a named expert sign-off before a child builds anything, and who is it?**

`meta_prompt/meta_curriculum_builder.prompt.v6.md:8` states the goal as a run that
*"needs nobody watching."* Every serious actor found keeps a qualified human as the
terminal gate, including those with a commercial interest in claiming otherwise. In the
EU, EN 71-1 makes instructions for safe use and foreseeable misuse a compliance
deliverable, so for this curriculum the question is not purely editorial.

**Phase 6 does not start until both are answered.** Phases 0–5 and 7 do not depend on
them.
...
| 9 | Acceptance | code decides; the human sign-off gate, where the curriculum declares one |
...
| 9 Acceptance | `architecture.v1.md:6-11`, `routing.v1.md:23-25` | keep; add the sign-off gate from §4(b) |
...
— plus simulation. **The engine refuses to start when the declaration is missing or its
...
**Phase 6 — one unit** — *blocked on §4.* L01 of `arduino_kit`, generated, rendered,
read by a person.
...
2. **If Phase 6 has not produced one readable unit, no further specification work is
   authorised** — not a v3 of this plan, not another research round. The next artifact
...
5. §4(a) and §4(b) answered by a human, recorded with the option and the date.
6. One unit of `arduino_kit` exists, rendered and read.
...
Building the controller. Authoring the vetted circuit library, which follows the §4(a)
decision rather than preceding it. Changing
`curricula/arduino_kit/arduino_kit_curriculum.v4.yaml` — the sequence is human-authored
and the evidence says it stays that way. The registry generalisation, which belongs to
