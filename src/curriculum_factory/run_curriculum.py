...
from curriculum_factory.langgraph_factory.graph import build_curriculum_factory_graph, build_runtime_context
from curriculum_factory.langgraph_factory.nodes.inputs import (
...
PROG = "python3 -m curriculum_factory.run_curriculum"
CONTRACT_VERSION = "1"
...
def _capability_forbidden_paths(engine_root: Path) -> list[Path]:
    return [path for path in (engine_root / "pyproject.toml", engine_root / "src" / "curriculum_factory") if path.exists()]
