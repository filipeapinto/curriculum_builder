...
2. **This report publishes the manifest's own digest**, in §2.4. The manifest therefore
   ...
   the other.
3. **The QA gate covers this report.** It hashes the artifact every round, folds that
   into its hash chain, records it in Codex's own rollout file, and `verify` re-checks
   that the artifact which passed is byte-identical to the one reviewed. That binding is
   stronger than any digest this checkpoint could publish about itself, and it is the one
   the executing agent cannot forge.

The only other exclusion is `checkpoints/P03/QA/`, which is gate-owned. §2.4 gives the
exact exclusion table and counts.

An earlier version of this checkpoint tried to close the same loop with a separate
`checkpoint_digest.json` anchor plus a `git write-tree` tree id. Round 1 of this session
found the resulting hole — six files vouched for by nothing, since the tree id was never
actually published — and it was right. The anchor has been deleted rather than patched.
...
| Artifact under review | **this file**, `P03_recovery_checkpoint.v4.md` (v1–v3 superseded — §1.2) |
| Execution journal | `plans_internal/refactor_repo/execution/P03/execution_log.jsonl` — 50 records, **zero unclosed starts** |
...
| | |
|---|---|
| Total paths in delta vs `ccacad3` | **243** |
| Added | 134 |
| Renamed | 68 |
| Modified | 40 |
| Deleted | 1 |
...
**`sha256(evidence/digest_manifest.json)` = `2744c3ba6fd9861007260e61709b38ee16afe605ed2fbb053ae528d82e1c8517`**
...
that hashes it cannot each be inside the other — and this report's own digest is
established by the QA gate instead: the gate hashes the artifact every round, folds it
into its hash chain, records it in Codex's own rollout file, and `verify` re-checks that
the artifact which passed is byte-identical to the one reviewed.
...
| `P03_recovery_checkpoint.v4.md` (this report) | the QA gate's hash chain, Codex's rollout witness, and `verify`'s byte-identity re-check |
...
**What changed here, and why.** Round 1 of this session found that six relied-on
deliverables — the four ledger files, the manifest, and a `checkpoint_digest.json` anchor
— were excluded from coverage while the fallback they were said to rely on, a
`git write-tree` tree id, was not published anywhere. That was a real hole, and the
finding was correct. It is fixed structurally rather than patched: the anchor file is
deleted, the ledger files are now covered by the manifest like everything else, and the
remaining two-file loop is closed by the gate's own chain instead of by a value this
checkpoint publishes about itself.

`deprecated/` is *not* excluded: its current contents — three superseded checkpoint
...
1. **A further round relocates this file.** If the gate returns findings, `round` moves
   `P03_recovery_checkpoint.v4.md` into `deprecated/`. It cannot be pre-staged without
...
`deprecated/P03_recovery_checkpoint.v2.md` and `deprecated/P03_recovery_checkpoint.v3.md`,
moved by hand so that no superseded version sits in the live slot at freeze time and
every one of them is digest-covered at a stable path — are staged, appear in the ledger,
and are covered by the manifest.
...
| Gate | State |
|---|---|
| `require_valid_journal` | 50 records, execution-log-v2 shape |
...
| `require_authorized_paths_only` | **0 unauthorized** of 243 paths |
