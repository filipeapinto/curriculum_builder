...
def main() -> int:
    ...
    validations = [
        ...
        command_record(["python3", "runtime/run_curriculum.py", "--curriculum", "curricula/arduino_kit", "--test-static"], engine),
        command_record(["python3", "runtime/run_curriculum.py", "--curriculum", "curricula/arduino_kit", "--test-simulated-all"], engine),
    ]
    ...
    task_changes = [line for line in current_status.splitlines()
                    if line[3:].startswith("runtime/") or line[3:].startswith("tests/runtime/")
    ...
    audit = {
        ...
        "write_scope": {"task_owned_changes": task_changes,
                        "allowlist": ["runtime/**", "tests/runtime/**", "three unchanged policy files"],
